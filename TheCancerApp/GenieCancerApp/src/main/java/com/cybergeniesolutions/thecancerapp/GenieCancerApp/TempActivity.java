...
public class TempActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String fileName = "bodytemperature.pdf";
    ...
    private void fillData() {
        ...
        Cursor c = db.fetchAllTemps();
        tempList.clear();
        Log.v(TAG, "cursor = " + c.getCount());
        while (c.moveToNext()) {
            Temperature temp = new Temperature(c.getString(c.getColumnIndex(db.T_DATE)),
                    c.getString(c.getColumnIndex(db.T_TIME)), c.getString(c.getColumnIndex(db.T_DAY)),
                    c.getString(c.getColumnIndex(db.T_TEMP)), c.getInt(c.getColumnIndex(db.T_UNIT)), c.getLong(c.getColumnIndex(db.KEY_ROWID)));
            tempList.add(temp);
            Log.v(TAG, "temp = " + temp.getTemp());
        }
    ...
    private void createTemp() {
        Intent i = new Intent(this, TempEditActivity.class);
        i.putExtra("Page", "Add");
    ...
    public void createPDF(File file) throws IOException, DocumentException {
        ...
        preface.add(new Paragraph("Body Temperature Records", catFont));
    ...
    private PdfPTable getRecordTable() {
        ...
        cell.setColspan(1);
        table.addCell(cell);
        cell = new PdfPCell(new Phrase("Body Temperature"));
        cell.setColspan(1);
        table.addCell(cell);
        ...
        if(c != null){
            ...
            while (c.moveToNext()){
                ...
                String tempString = c.getString(c.getColumnIndex(db.T_TEMP));
                String unitString = c.getString(c.getColumnIndex(db.T_UNIT));
                if (unitString.equals("0"))
                    table.addCell(tempString + " " + TempActivity.this.getResources().getString(R.string.radio_temp_c));
                else
                    table.addCell(tempString + " " + TempActivity.this.getResources().getString(R.string.radio_temp_f));
            }
        }
